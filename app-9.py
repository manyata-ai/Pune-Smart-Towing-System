...
DATA_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "Pune_With_Real_Features_v5.csv")
...
def extract_features(text):
    ...
    return {
        ...
        "zero_tolerance_kw": word_in_text(
            t, ["zero tolerance", "zero-tolerance", "strictly prohibited", "strictly banned", "absolute ban"]
        ),
...
def train_model(df: pd.DataFrame):
    ...
    data["zero_tolerance_kw"] = data["Road Name"].apply(
        lambda x: word_in_text(
            x, ["zero tolerance", "zero-tolerance", "strictly prohibited", "strictly banned", "absolute ban"]
        )
    )
...
def predict_risk(model, le_div, le_date, data, road_name_input, division_input, date_type_input):
    """Replicates the original notebook's prediction flow (cell 18)."""
    match = data[data["Road Name"].str.contains(re.escape(road_name_input), case=False, na=False)]

    if not match.empty:
        division_name = match.iloc[0]["Division Name"]
        input_text = match.iloc[0]["Road Name"]
        yard_name = match.iloc[0]["Yard Name"]
    else:
        ...
        if not road_name or not road_name.strip():
            ...
            with rc1:
                ...
                st.markdown(
                    ...
                    **Match found in database:** {"Yes ✅" if result['match_found'] else "No — used manual division ⚠️"}<br>
...
# ============================================================================
    ...
    with yf1:
        ...
        if road_choice_yf == "✏️ Type a custom road / location":
            road_name_yf = st.text_input("Enter road / location name", key="yf_road_text")
        else:
            road_name_yf = road_choice_yf
    with yf2:
        division_input_yf = st.selectbox("Area / Division", DIVISIONS, key="yf_division")
    ...
    if find_clicked:
        if not road_name_yf or not road_name_yf.strip():
            ...
            match = df[df["Road Name"].str.contains(re.escape(road_name_yf), case=False, na=False)]
            if not match.empty:
                division_name = match.iloc[0]["Division Name"]
                yard_name = match.iloc[0]["Yard Name"]
                found = True
                ...
                st.markdown(
                    ...
                    <div class="yard-card">
                        ...
                        <div style="margin-top:0.8rem;">
                            ...
                            {'Matched from database' if found else 'Estimated from division'}
                            </span>
                        </div>
                    </div>
                    """,
